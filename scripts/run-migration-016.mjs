/**
 * Applies migration 016: adds show_sales_count to the products table.
 *
 * Usage:
 *   node scripts/run-migration-016.mjs
 *
...
async function columnExists() {
  const { error } = await supabase
    .from('products')
    .select('id, show_sales_count')
    .limit(0)
  return !error
}

async function main() {
  console.log('=== Migration 016: show_sales_count column on products ===\n')
  ...
  console.log('─────────────────────────────────────────────────────────')
  const sql = readFileSync(resolve(root, 'supabase/migrations/016_add_show_sales_count.sql'), 'utf8')
